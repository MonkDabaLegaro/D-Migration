...
public sealed class DockerDesktopInventoryProvider(IDockerDesktopMigrationHost host, string destinationDrive) : IInventoryProvider
{
    ...
    public Task<IReadOnlyList<InventoryItem>> ScanAsync(CancellationToken cancellationToken = default)
    {
        ...
        var notes = automatic
            ? "Docker Desktop WSL2 con canal de reubicación verificable."
            : "Reubicación asistida: Docker Desktop no expone un canal automatizable soportado en esta instalación.";
        InventoryItem item = new(
            "docker-desktop-wsl2", "docker-desktop", "Docker Desktop data", root, 0, "docker",
            RiskLevel.High, MigrationStrategy.DataRootMigration,
            Path.Combine(destinationDrive, "Datos", "Docker", "wsl"), automatic, notes);
...
public sealed class DockerDesktopMigrationProvider(IDockerDesktopMigrationHost host) : IMigrationProvider
{
    ...
    public Task<MigrationOperationResult> ValidateAsync(MigrationStep step, CancellationToken cancellationToken = default)
    {
        ...
        return imagesMatch && volumesMatch
            ? Ok("Inventario Docker validado: imágenes y volúmenes coinciden.")
            : Fail("El inventario Docker del destino no coincide con el origen.");
    }
